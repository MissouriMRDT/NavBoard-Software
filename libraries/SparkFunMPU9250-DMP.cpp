...
void MPU9250_DMP::calibrateMag(int time)
{
	int timer = millis();
	while ((millis() - timer) < time)
	{
		updateCompass();
		if (magMin[0] >= mx) magMin[0] = mx;
		if (magMax[0] <= mx) magMax[0] = mx;
		if (magMin[1] >= my) magMin[1] = my;
		if (magMax[1] <= my) magMax[1] = my;
		if (magMin[2] >= mz) magMin[2] = mz;
		if (zMax <= mz)
		{
		zMax = mz;
		}
