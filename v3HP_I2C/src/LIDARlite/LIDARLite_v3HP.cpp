...
/*------------------------------------------------------------------------------
  ...
  lidarliteAddress: Default 0x62. Fill in new address here if changed. See
    operating manual for instructions.
------------------------------------------------------------------------------*/
uint32_t LIDARLite_v3HP::readDistance( uint8_t *distance2, uint8_t lidarliteAddress)
{
    
    uint8_t dataBytes[2] = {55, 55};
	...
	uint32_t  distance = dataBytes[0] * 255 + dataBytes[1];
	//Serial.println(distance);

    // Shift high byte and add to low byte
    //distance = dataBytes[0] * 255 + dataBytes[1];
	//Serial.println(distance);
	//Serial.println("Exit");
    return (distance);
                           ...
                           uint16_t numBytes, uint8_t lidarliteAddress)
{
    ...
    Wire.beginTransmission((uint8_t) lidarliteAddress);
                          ...
                          uint16_t numBytes, uint8_t lidarliteAddress)
{
    ...
    Wire.beginTransmission((uint8_t) lidarliteAddress);
    Wire.write((uint8_t) regAddr); // Set the register to be read

    // A nack means the device is not responding, report the error over serial
    nackCatcher = Wire.endTransmission(false); // false means perform repeated start
	//Serial.println(nackCatcher);
    if (nackCatcher != 0)
    {
        Serial.println("> nack");
    }
